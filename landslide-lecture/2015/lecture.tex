...
\definecolor{thread3}{RGB}{255,87,130}
	...
	\begin{center}
		\begin{tabular}{llll}
			\begin{tabular}{l}
				...
				\texttt{~~~~WAIT(cvar,~mx);} \\
				\texttt{work~=~claim\_work();} \\
				\\
				\texttt{UNLOCK(mx);} \\
				\texttt{assert(work~!=~NULL);} \\
			\end{tabular}
			& & &
			\begin{tabular}{l}
				Producer thread \\
...
\begin{frame}{Thread Interleavings (``good'' case)}
	\begin{center}
		\begin{tabular}{|l|l|l|}
			...
			& \small \texttt{LOCK(mx);} & \\
			& \small \texttt{give\_work(work);} & \\
			& \small \texttt{SIGNAL(cvar);} & \\
			& \small \texttt{UNLOCK(mx);} & \\
			
			\small \texttt{work~=~claim\_work();~} & & \\
			\small \texttt{UNLOCK(mx);} & & \\
			\small \texttt{//~got~the~work} {\large \smiley}& & \\
...
\begin{frame}{Thread Interleavings (race condition)}
	\begin{center}
		\begin{tabular}{|l|l|l|}
			...
			\small \texttt{work~=~claim\_work();~} & & \\
			\small \texttt{UNLOCK(mx);} & & \\
			\small \texttt{//~work~was~stolen} {\large \frownie}& & \\
			\hline
		\end{tabular}
	\end{center}
\end{frame}
...
\section{Landslide for the People}
...
% IRB approved, plan on publishing collected data, anonymized
% Expect to spend X time
% Participation has no impact on grade but you will find bugs
