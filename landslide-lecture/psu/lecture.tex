...
\begin{frame}{Case Study 1}
	\begin{center}
			...
			\begin{tabular}{l}
				...
				\hline
				\\
				\texttt{/* int atomic\_fetch\_add(volatile int* obj, int arg); */} \\
				\texttt{.global atomic\_fetch\_add} \\
				\texttt{atomic\_fetch\_add:} \\
				\texttt{~~mov 4(\%esp), \%ecx~~~~~~// obj} \\
				\texttt{~~mov 8(\%esp), \%edx~~~~~~// arg} \\
				\texttt{~~mov~~(\%ecx), \%eax~~~~~~// return old value of obj} \\
				\texttt{~~lock xadd \%edx, (\%ecx) // atomically increment obj} \\
				\texttt{~~ret} \\
			\end{tabular}
			}
		\pause
...
\begin{frame}{Case Study 2}
	\begin{center}
		...
		\begin{itemize}
			\item See {\bf Synchronization} lecture!
...
\begin{frame}{Preemption Point Example ({\tt atomic\_fetch\_add()})}
	\fbox{
	\begin{tabular}{rll}
		\multicolumn{3}{l}{\texttt{/* int atomic\_fetch\_add(volatile int* obj, int arg); */}} \\
		1 & \texttt{.global atomic\_fetch\_add} \\
		2 & \texttt{atomic\_fetch\_add:} \\
		3 & \texttt{~~mov 4(\%esp), \%ecx} \\
		  & & \footnotesize (preemption point A) \\
		4 & \texttt{~~mov 8(\%esp), \%edx} \\
		  & & \footnotesize (preemption point B) \\
		5 & \texttt{~~mov~~(\%ecx), \%eax} \\
		  & & \footnotesize (preemption point C) \\
		6 & \texttt{~~lock xadd \%edx, (\%ecx)} \\
...
\begin{frame}{Preemption Point Example ({\tt atomic\_fetch\_add()})}
	...
	Here, only preemption point C will break atomicity. {\bf ~}\\
...
\begin{frame}{About The Project}
	...
	\begin{itemize}
		\item TAed CMU OS (same thread project) for 3 semesters %during undergrad
...
\begin{frame}{Big Picture: Execution Tree Exploration}
	...
	\begin{itemize}
		...
		% \item Implemented with fork() in Bochs
...
\begin{frame}{Previous Semesters in CMU's OS class}
	...
	%% F17 lecture - stats up to S17
	%% 91 {\em deterministic} bugs (e.g. swexn, initialization)
	% S18 lecture - stats up to F17
	...
	%% F17 lecture - stats up to S17
	%% 113 distinct {\em non-deterministic} bugs (among 44 groups)
	% S18 lecture - stats up to F17
	122 distinct {\em non-deterministic} bugs (among 49 groups)
	\begin{itemize}
		\item 36 groups (73\%) fixed $\ge 1$ such bug
		%\item 33 groups (75\%) fixed $\ge 1$ such bug
		...
		%\item 24 groups (55\%) fixed {\em all} such bugs
...
\begin{frame}{User Study - How to Participate}
	...
	\begin{itemize}
		...
		\item Complete sign-up form online to get further instructions
			\begin{itemize}
				\item {\tt https://tinyurl.com/landslide-psu-s18}
