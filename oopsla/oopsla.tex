...
\author[Ben Blum]{{\bf Ben Blum}, Garth Gibson} %\textsf{bblum@cs.cmu.edu} \\ Garth Gibson \textsf{garth@cs.cmu.edu}}
...
% Say, "As the first sentence of any related paper in this field will tell you, "concurrency bugs are notoriously hard to find and debug", so stateless MC is a technique to make it less painful."
...
\begin{frame}{Implementation}
	...
	\begin{itemize}
		...
		\item Wind River Simics provides instruction/memory-level tracing
			% Say "although the same thing could be achieved with compiler instrumentation"
...
% Q: What's with all the 'unknowns' in wait-simple for SSSMC-Everywhere?

% A: Those test cases weren't able to compute a state space estimate because
% they couldn't even reach the end of the first execution to be tested. Unlike
% the other 8 tests, wait-simple is testing both kernel and userspace code at
% once, which compounds the effect of putting a PP on every single memory access.

% Q: Inconsistencies between QS/Everywhere on verified vs bug-found tests,
% doesn't that serve as a counterexample to your soundness of verification?

% A: I need to look into those, but to make a guess, there are a few heuristics
% at play which could introduce some unsoundness in either trial. For example,
% our heuristic infinite loop check looks at the structure of the execution
% tree, comparing the number of PPs against past branches, which could be
% falsely tripped by all the PPs which Everywhere adds. On the other hand,
% SSSMC-Everywhere excludes threads' memory accesses to their own stacks, which
% could potentially be data races depending on the implementation of certain
% sync primitives.

% Q: MCR?

% A: MCR is a DPOR alternative which upgrades its ability to find reductions.
% It's still a strategy for exploring a single state space, so it's perfectly
% compatible with Quicksand. Whether or not the performance improvements are
% orthogonal is a matter for future work.
