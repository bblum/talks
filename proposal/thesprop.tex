...
\newcommand\llitem{\item[\llama{1em}]}
...
\begin{itemize}
	...
	\item {\bf Broader impact (Chapter \ref{chap:lipservice}).}
		...
		To prove these techniques are relevant beyond CMU's walls, I will extend Landslide to handle Pintos kernel projects from other universities \cite{pintos}. %and ``real-world'' Linux programs.
...
Bear in mind that for the first half of 2017, I'll be working with a full-time research assistant,
David Vernet, on the transactional memory and porting Landslide projects.

\begin{itemize}
	\item 2017 January: Submit thesis proposal, begin work on TM project (Section~\ref{sec:tm}). Also submit revised IRB proposal for planned user studies.
	\item 2017 ???uary: Present thesis proposal (whenever the committee is free).
	\item 2017 February: Develop ``grading'' TA user study and deploy it in 15-410 (Section~\ref{sec:grading}).
	\item 2017 March-April: Finish implementation/evaluation of TM; write a paper about it.
	\item 2017 April-June: Begin ``port Landslide to VMware or Bochs'' project (Section~\ref{sec:pintos-todo}).
	\item 2017 June-July: Resubmit paper about 410 user studies (Section~\ref{sec:studence}) (previously rejected from SIGCSE). Also run ``variance'' Quicksand experiments (Section~\ref{chap:quicksand}).
	\item 2017 August-October: Polish and deploy Pintos user study (Section~\ref{sec:pintos-todo}).
	\item 2017 September-December: Write thesis. Also deploy 410 TA grading study once more for more data.
	\item 2017 December: Defend \& graduate.
\end{itemize}
