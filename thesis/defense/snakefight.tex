...
\begin{frame}{Example}
	\begin{center}
		\begin{tabular}{l|l}
			...
			\\
			{\tt \hilight{darkorange}{\_xbegin}() == FAIL} \\
...
\begin{frame}{Evaluation - Verification}
	$K,N$ = $K$ threads, each $N$ iterations of test logic
	\linegap
...
\begin{frame}{Happens-Before Example}
	\begin{center}
		\begin{tabular}{ll}
			...
			\texttt{\hilight{red}{x++;}}    & \\
			\texttt{mutex\_lock(m);}	& \\
			\texttt{\hilight{darkblue}{y = true;}}      & \\
			\texttt{mutex\_unlock(m);}      & \\
				& \texttt{mutex\_lock(m);} \\
				& \texttt{\hilight{darkblue}{bool tmp = y;}} \\
				& \texttt{mutex\_unlock(m);} \\
				& \texttt{\hilight{darkblue}{if (tmp)}~\hilight{red}{x++;}} \\
		\end{tabular}
		\linegap
...
\begin{frame}{References}
	...
	\begin{itemize}
		...
		\item {\bf [Simsa '13]}: Systematic and Scalable Testing of Concurrent Programs.
			CMU-CS-13-133. December 2013.
		\item {\bf [Coons '13]}: Katherine Coons et al. Bounded Partial Order Reduction. OOPSLA 2013.
		\item {\bf [vamsikc '14]}: leveldb-tsx. https://github.com/vamsikc/leveldb-tsx/
