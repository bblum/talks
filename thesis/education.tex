...
In total, 28 students (or pairs thereof, in the case of CMU and U. Chicago) answered the survey:
...
The survey results were overwhelmingly positive, and largely speak for themselves:
% TODO: corroborate this with snapshot data?
students reported being able to both diagnose and verify fixed the vast majority of Landslide's reported bugs,
...
are that of what kind of false positives Landslide (regrettably) reported % oopsative mood
(CMU and PSU only),
...
\begin{enumerate}
	\item Landslide complained of a nonexistent {\tt MAGIC\_BREAK} (Simics debugging function),
		...
		This arose because of confusion between the test program's and the shell's address spaces,
...
\resume{enumerate}
	...
	\item Students who initialized child threads with a base pointer value of {\tt 0xffffffff} observed Landslide crashes,
		as it attempted to stack trace through that address (accessing memory that wrapped around the address space).
...
{\bf Reasons worthwhile.}

{\bf Interface suggestions.}
