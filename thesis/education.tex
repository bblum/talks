...
% TODO: look at this spacing hack again on fresh eyes and see if it rly improves matters or not
\vspace{1em}
...
% fix spacing between paragraphs, since sect 5.1 header starts next page
\vspace{3em}
...
is available at
\url{https://www.cs.cmu.edu/~410/landslide/psu-lecture.pdf},
...
\url{https://www.cs.cmu.edu/~410/landslide/landslide-guide-p2.pdf}
(CMU version)
and at % these "at"s can optionally be adjusted to make url line breaks better
\url{https://www.cs.cmu.edu/~410/landslide/landslide-guide-psu.pdf}
(PSU version)
and at
...
\begin{itemize}
	...
	\item {\tt rwlock\_downgrade\_read\_test}:
		...
		\revisionminor{which should atomically convert a lock held in exclusive write mode to shared read mode}.
...
\noindent The tests can all be viewed at
%\url{https://github.com/bblum/landslide/tree/master/pebsim/p2-basecode/410user/progs}.
\url{https://github.com/bblum/landslide/tree/master/pebsim/p2-basecode/landslide-friendly-tests}.
...
\begin{itemize}
	\item Finds the declaration of {\tt ready\_list}, the scheduler runqueue declared by the basecode,
		...
		\revisionminor{emits macros to configure {\tt is\_runqueue()} to handle
		certain common alternate approaches}
		(part of the patch described below).
		Either way,
		defines a function {\tt get\_rq\_addr()} to return the address of the %(first)
		list.
...
\begin{itemize}
	...
	\item {\tt alarm-simultaneous}:
		Modified to be Landslide-friendly from the basecode, also for {\em threads}.
...
\Cref{tab:photo-of-ze-studence} shows the student participation rate across semesters
...
\revisionminor{(inspecting their code if necessary,
as multiple traces may refer to the same bug),}
...
\Cref{tab:cmubugs} shows these statistics for each semester.
...
Though unscientific,
this provides some assurance that \Cref{tab:cmubugs}'s count of concurrency bugs is accurate,
...
\Cref{fig:photo-of-ze-studence} shows the distribution of project grades in CMU 15-410
...
%We see in \Cref{fig:photo-of-ze-studence} that, indeed,
{\bf Results.}
Overall,
...
\Cref{tab:table-of-ze-studence} presents more detailed statistics from these six distributions.
...
\footnote{I choose not to correct for multiplicity among these four comparsons \revisionminor{as} I do in the next section
...
However, the difference being smaller in P3 than in P2
suggests, albeit informally,
...
Based on the results from \cref{sec:education-eval-bugfinding},
...
as non-users.
\Cref{tab:eval-common-bugs} presents the results.
...
% not-to-do: some reference to future work about focused testing on small code diffs? talked with some industry ppl about this sometimes
...
% corroborate this with snapshot data? -- haha, never
...
Comparing the survey response distributions between CMU versus PSU and U. Chicago (\Cref{fig:survey}),
...
%(tempering the difficulty of P2 to make it more accessible (\cref{sec:overview-psu})),
...
% u can verify the bug just by taking survey graph #1 and comparing to landslide snapshots (sep. by groups who answered survey)
% ^ haha, never
% anything else?
...
the participation rate rose to 58\% (\Cref{tab:photo-of-ze-studence})
...
\begin{enumerate}
	...
	\item \revision{{\em In addition to finding bugs, did you manage to fully verify your code under
		any tests by letting Landslide complete all state spaces before reaching the specified time out?}
		This would measure how thoroughly students understood the underlying research technique,
...
% report on participation %s

\revision{This chapter has introduced Landslide as an educational tool
in the undergraduate operating systems classes at CMU, U. Chicago, and PSU.
I have made the following contributions:

\begin{itemize}
	\item A lecture, polished over seven semesters,
		for introducing stateless model checking as a concurrency testing approach
		in high-level undergraduate classes
		(with many slides admittedly specific to Landslide)
		(\cref{sec:education-pebbles-recruiting}).
	\item A system for automatically instrumenting student thread library (CMU, PSU) and kernel (U. Chicago)
		project implementations to be compatible with stateless model checking
		(\cref{sec:education-pebbles-instrumentation},
		\cref{sec:education-pintos-instrumentation}).
	\item An experimental procedure for evaluating debugging and verification tools
		as student-accessible teaching aids,
		including a survey for measuring the more intangible human experience
		(\cref{sec:education-survey-pebbles},
		\cref{sec:education-survey-pintos}).
	\item A large evaluation spanning nine semesters of operating systems classes
		(\cref{sec:education-eval-bugfinding}),
		two of which were held at other universities besides CMU,
		in which 145 students used Landslide,
		a majority of which found bugs (\cref{sec:education-eval-bugfinding}),
		and a vast majority of which expressed via the survey that they had a good experience
		(\cref{sec:education-eval-survey}).
	\item A thorough statistical analysis of Landslide's effect on student submission quality
		(\cref{sec:education-eval-grades}),
		evaluating both impact on ultimate project grades
		and student tendency to submit any of four well-known case study bugs.
		Ultimately, while positive trends were observed,
		this study concluded that a statistically-significant conclusion could not be drawn
		while simultaneously compensating for several obvious sources of possible bias.
		Nevertheless, this level of statistical rigor is not yet widespread in computer science research,
		so I would hope this study may set a precedent that help mature the field.
	\item Concrete action taken to establish Landslide
		as a permanent fixture of 15-410's grading process at CMU,
		student-oriented user guides should the course staff wish it to remain available as a debugging tool,
		as well as general recommendations for any Pintos-based OS class
		at other universities to use it for grading as well
		(\cref{sec:education-future-use}).
\end{itemize}
}

Human subjects research is inherently messy.
Each %of the previous section's
\revisionminor{individual}
