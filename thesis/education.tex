...
Figure~\ref{Fig:photo-of-ze-studence} shows some resulce
...
The $p$ values are calculated in R \cite{r-lang} using Fisher's exact test \cite{fishers-exact-test},
treating each row as an independent 2x2 contingency table,
and dividing the significance cutoff by the number of bugs to conservatively account for multiplicity \cite{xkcd-jellybeans}.
...
Reaching over half the class could alone be seen as a major step in mitigating said selection bias.
...
and the proportion of women was in fact somewhat higher than the overall gender ratio.
Any more scientific analysis of such statistics was deemed beyond the study's scope for now.
