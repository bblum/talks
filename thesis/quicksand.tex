...
\subsubsection{Avoiding redundant work}
...
\subsubsection{Preemption point subsets}
...
% but also include .....
Any data-race candidates identified in advance could heuristically be included in Quicksand's initial seed PP sets
(\cref{sec:quicksand-initial-pps}),
...
although it is opaque to the user about which parts of her code were actually tested.
...
Iterative Deepening offers a clear benefit to the expert user
via the {\tt within\_function} command,
which enables her to restrict a test's scope to only the modules of a codebase she wishes to test.
