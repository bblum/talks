...
\begin{enumerate}
	...
	\item Does testing alternate interleavings with model checking improve the accuracy of data race analysis?
		\begin{enumerate}
			...
			\item Does Quicksand find data-race bugs that single-execution Pure or Limited Happens-Before would miss?
		\end{enumerate}
\end{enumerate}
...
I tested P2s with 6 multithreaded programs:
...
I used the {\tt without\_function} command to exclude {\tt thr\_create}, {\tt thr\_exit}, and {\tt thr\_join} preemption points,
and for \mxtest I enabled {\tt TESTING\_MUTEXES} (\cref{sec:landslide-dynamicconfig}).
I tested Pintoses with 3 programs from the class's provided test suite:
...
\begin{itemize}
	...
	\item {\bf QS-Sync-Only}: Quicksand with initial preemption points only \cref{sec:quicksand-initial-pps},
...
Hence, I optimistically gave control experiments a linear speedup of 10 hours per test with 1 CPU;
...
although Quicksand's inherent parallelism is a convenient benefit unto itself.
