...
Note that Iterative Deepening is a wrapper algorithm around stateless MC.
...
% TODO: check spcaing between defs/lemmas/etc and surrounding text
...
\Cref{fig:dowefindbugsfaster}(a)
is the main, resource-fair comparison by CPU-time,
while \Cref{fig:dowefindbugsfaster}(b) shows
the wall-clock time until each bug was found.
...
\Cref{fig:dowefindbugsfaster}(b) gives Quicksand full credit for its inherent parallelism,
...
\Cref{tab:drbugs} provides more detail on each of the bugs shown in \Cref{fig:dowefindbugsfaster},
broken down by test case.
The left half shows the number found by each experimental approach,
with the totals of each column corresponding to the values at $x=10$ hours in \Cref{fig:dowefindbugsfaster}.
...
\begin{table}[h]
	...
	\begin{tabular}{r||c|c||c}
		{\bf Bound} & {\bf SSS-MC-ICB} & {\bf SSS-MC-Shared-Mem} & Prior work \cite{chess-icb} \\
		\hline
		0         & 2     & 1     & 3 \\
		1         & 82    & 86    & 7 \\
		2         & 16    & 32    & 5 \\
...
in the same style of graph as \Cref{fig:dowefindbugsfaster}.
...
\begin{figure}[t]
	\begin{center}
	\includegraphics[width=0.71\textwidth]{../proposal/totalverifs-v2.pdf}
	\end{center}
...
Prior work has proposed Probabilistic Concurrency Testing %(PCT)
\cite{randomized-scheduler},
which attempts to quantify the probability
that a bug may remain in some untested interleaving after a time-out,
although is opaque to the user about which parts of her code were actually tested.
Meanwhile, ICB proposes to inform the user
of the maximum number of preemptions that were used to test any individual interleaving,
under the assurance that most bugs are likely to be found with fewer preemptions (\cref{tab:icb-bounds}).
Quicksand instead reports which subsets of preemption points resulted in state spaces that did complete in time,
...
% of Table~\ref{tab:drbugs}. % thesis - now split into sep table
...
On the other hand, attempting to maximize the raw number of completed state spaces
may not be the most user-friendly way to present partial verification results.
For example, included in these numbers are small state spaces which are subsets of other state spaces also completed;
the user need not examine both subset and superset separately.
Future work could conduct user studies to determine the most effective way to present these partial results
from a software development perspective,
such as described in \cref{sec:future-friendly}.
...
Beyond finding new bugs and completing full verifications with data-race PPs,
...
\begin{table}[t]
	...
	Among those, ``Benign DRs'' counts how many were refuted as non-failing,
	while ``Untested DR PPs'' counts how many could not be checked in the time limit.
...
QS-Pure-HB is even more dependent thereupon, %on nondeterministic data-race PPs,
%requiring nondeterministic data-race PPs
...
\begin{figure}[t]
	\begin{center}
	\includegraphics[width=0.85\textwidth]{../proposal/nondets.pdf} % TODO: get some gf colors on here
	\end{center}
