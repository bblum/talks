...
\begin{itemize}
	...
	\item {\tt sched\_update\_kern\_state\_machine()} ({\tt schedule.c})
		will:
		\begin{itemize}
			\item handle the reference kernel's invocation of {\tt sched\_unblock()} within {\tt cond\_signal()}
				as a signal event for happens-before analysis.
			\item when a {\tt vanish}ing thread is about to delete its Simics symbol table object,
				save a ``pre-vanish'' stack trace,
				which {\tt splice\_pre\_vanish\_trace} ({\tt stack.c}) will then splice into any subsequent stack traces by that thread,
