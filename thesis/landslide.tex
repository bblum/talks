...
there's no guarantee $J$ would get marked for exploration
from any other iteration and/or any other pair of conflicting transitions.
Even though restricting the state space to a certain maximum preemption bound is already unsound
in terms of losing full concurrency coverage,
failing to test even $J$ would be a failure of DPOR itself
to soundly prune the already-reduced state space defined by the bound.
Hence the need for BPOR, to ensure that if such an alternative $J$ to $I$ exists within the bound,
$J$ gets marked for exploration immediately.
