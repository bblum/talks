...
computing memory conflicts, computing happens-before,
...
\begin{itemize}
	\item {\bf Enabled}: A transition $t$ is enabled in a state $s$ when a state $s'$ exists such that $s \xrightarrow{t} s'$ exists.
		In systems terms, the scheduler at state $s$ considers the thread corresponding to $t$ to be runnable.
	\item {\bf Dependent}: Two transitions $t_i$ and $t_j$ are dependent if
		...
		\end{itemize}
		In systems terms, either $t_1$ and $t_2$ belong to the same thread,
		or the execution of $t_1$ at $s$ causes $t_2$'s thread
		to change state from blocked to runnable or vice versa.
		Landslide computes this relation in {\tt enabled\_by()} in {\tt save.c}.
		\footnote{The original definition includes a second criterion that, from $s$,
		the executions $t_1 t_2$ and $t_2 t_1$ result in identical states;
		this captures the memory independence relation which I defer to the next subsection.}
	\item {\bf Happens-Before}: The happens-before relation for a transition sequence $S = t_1 \dots t_n$
		...
		\end{enumerate}
		Landslide computes this relation in the aptly-named {\tt compute\_happens\_before()} in {\tt save.c}.
\end{itemize}
...
and hence candidates for new thread interleavings to test.
...
The memory conflict relation expresses when two thread transitions are dependent,
...
and then empties that set for the next thread transition ({\tt shimsham\_shm()} in {\tt save.c}).
...
which should conflict with accesses to any address therein.
