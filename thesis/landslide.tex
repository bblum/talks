			...
			\item At each instruction:
				\begin{enumerate}
					\item Update the {\em scheduler},
						a state machine which depicts the runnable and/or blocked
...
The HEAD commit at the time of writing is 5e4082a.
In the event of a GitHub outage or the above link otherwise not working,
a backup of the repository
checked out at that commit
may be found at
\url{https://www.cs.cmu.edu/~410/landslide/landslide.tar.bz2}.
%
...
\url{https://www.cs.cmu.edu/~410/landslide/landslide-guide-kernel.pdf}.
...
\begin{itemize}
	...
	\item {\tt PREEMPT\_ENABLE\_VALUE=VAL} - value of the above variable when preemption is enabled
		...
		\footnote{These two options replace the deprecated {\tt kern\_ready\_for\_timer\_interrupt()} annotation from {\tt student.c} described in \cite[\sect{6.2.3}]{landslide}.}
...
is stored as a chain of preemption point nodes named {\tt struct nobe} defined in {\tt tree.h}.
...
Each {\tt nobe} stores the following information:
...
\footnote{\revisionminor{willan on-having switched to \cite{hhgg-reu}}}
...
and all modifications must go through {\tt modify\_pp()} ({\tt timetravel.h}) using a modification callback,
...
while for line numbers, {\tt pebsim/build.sh} (and {\tt pebsim/pintos/\allowbreak{}build.sh} for Pintos)
...
using the standard approach of following the base pointer chain
(not supporting code compiled with {\tt -fomit-frame-pointer} by doing anything clever
like understanding how much stack frame each function allocates),
...
\begin{itemize}
	...
	\item {\tt splice\_pre\_vanish\_trace()} ({\tt stack.c})
		...
		\revisionminor{unmapped all of its user memory,}
		%deleted its Simics symbol table object,
...
Landslide creates a new {\tt struct nobe} in the execution tree (\cref{sec:landslide-save}),
...
Landslide's implementation represents $S$ as a list of {\tt struct nobe}s, defined in {\tt tree.h},
...
Landslide need only store at each {\tt struct nobe} a list of threads
...
All other new interleavings are marked to explore later, %in their corresponding {\tt struct nobe},
...
%(with the previously-marked $\bigstar$2, now re-labeled as $\bigstar$3, having its subtree abbreviated). % old ver
\revision{(with the new $\bigstar$2 appearing in $\bigstar$1's subtree, to be explored before $\bigstar$3)}.
...
% TODO: check figure placement
% should be right below dpor-example-2, on the previous page
% i had to put ! not just ht to get it to show up correctly
\begin{figure}[ht!]
	\begin{center}
...
\subsubsection{Runtime (Recursive) estimation}
...
\footnote{\revisionminor{The pseudocode algorithms found in many conference papers
often fail to be straightforward to translate into usable implementations~\cite[Figure 5]{itg2}.}}
...
\footnote{A yield-blocking loop may depend on another thread's execution with no memory conflicts at all
simply by checking the return value to observe whether the yield was successful.
In principle, Landslide should check for such loops whenever the yielded-to thread's runnability changes;
at present, this is not implemented,
and instead leans on the false positive deadlock detector to wake the blocking thread up.}
...
\revision{This chapter has presented Landslide,
a stateless model checker for Pebbles and Pintos kernels and Pebbles userspace thread libraries.
For the most part, Landslide's contribution to the world of concurrency testing is as a feat of engineering
which combines many existing techniques from prior work in a single implementation.
These include:

\begin{itemize}
	\item Uses Dynamic Partial Order Reduction (DPOR) \cite{dpor}
		to prune equivalent interleavings while exploring the state space (\cref{sec:landslide-dpor}).
	\item Uses its simulator's memory tracing integration \cite{simics,bochs}
		to power data race analysis \cite{djit,fasttrack} (\cref{sec:landslide-datarace}),
		as well as DPOR.
		%and DPOR \cite{dpor} (\cref{sec:landslide-dpor}).
	\item Supports Preemption Sealing \cite{sealing} to configure the set of active preemption points
		and hence keep state space sizes manageable (\cref{sec:landslide-pps}).
	\item Extends DPOR with Recursive and Weighted Backtrack Estimation \cite{estimation}
		to predict overall ultimate state space size and completion time (\cref{sec:landslide-estimate}).
	\item Extends DPOR (at the user's option) with Iterative Context Bounding \cite{chess-icb}
		to heuristically uncover bugs faster should they exist,
		at the expense of overall completion time (\cref{sec:landslide-icb}).
\end{itemize}

However, over the years Landslide has also grown some convenience, accessibility, and or optimization features
that could be considered contributions in their own right.
...
\begin{itemize}
	...
	\item Implements the sleep set extension to DPOR \cite{partial-order-methods,dpor,optimal-dpor},
		...
		%, rather than just switching threads and seeing what happens
	...
	\item Provides automatic code instrumentation facilities,
		...
		that Landslide should treat as blocking, %a thread,
...
The following three chapters, \Cref{chap:quicksand,chap:education,chap:tm},
