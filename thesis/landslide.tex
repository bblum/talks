...
The Simics port uses Simics's proprietary API and is hence unlicensed and available upon request.
...
Despite being algorithmically worse in the size of the vector clocks,
I use the {\sc Djit+} rules for reads and writes rather than the {\tt FastTrack} ones;
because Landslide tests should be limited to few threads to manage the state space size,
$O(n)$ is usually in the single digits, so I optimize for code simplicity.

\begin{itemize}
	\item Reads and Writes (writes vs reads comparison already handled by DPOR) ({\tt memory.c})
	\begin{itemize}
	...
	\begin{itemize}
		\item \textsc{FT acquire}
			\begin{itemize}
				\item {\tt kern\_mutex\_\{,try\}locking\_done()} cases of {kern\_update\_state\_machine()}
				\item {\tt user\_mutex\_\{,try\}lock\_exiting()} cases of {user\_update\_state\_machine()}
		...
		\item \textsc{FT release}
			\begin{itemize}
				\item {\tt kern\_mutex\_unlocking()} case of {kern\_update\_state\_machine()}
				\item {\tt user\_mutex\_unlock\_entering()} case of {user\_update\_state\_machine()}
				\item {\tt sti} case of {\tt kern\_update\_state\_machine()} (Pintos only)
				\item {\tt cli}/{\tt sti} lock handoff case in {\tt sched\_update()} (Pebbles only)
			\end{itemize}
		\item \textsc{FT fork} - {\tt agent\_fork()}
		\item \textsc{FT join} - {\tt sched\_unblock()} case of {\tt kern\_update\_state\_machine()} (Pebbles only)
	\end{itemize}
\end{itemize}
