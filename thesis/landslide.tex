...
\begin{itemize}
	...
	\item Otherwise, there must be some third thread runnable at $s$
		...
		then, return all $I_{ik} = (t_1 \dots t_{i-1}, T_k)$ such that $T_k$ is runnable at $s$ and $T_k \ne T(t_i)$
		({\tt tag\_all\_siblings()}).
\end{itemize}

{\bf Tracking already-visited interleavings.}
Let $U(I_{ij})$ denote the sub-state-space (or sub-tree) beginning at the state
which results from executing $T_j$ after $t_1 \dots t_{i-1}$;
in other words, all sequences $S' = t_1 \dots t_{i-1}, t'_i, \dots t'_{n'}$ with $T(t'_{i+1}) = T_j$.
I use a depth-first search ordering,
...
({\tt struct hax\_child} in the code).
Hence Landslide's DPOR implementation's memory cost is $O(n*k)$,
$k$ being the average number of runnable threads at each preemption point.
