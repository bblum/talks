...
% TODO
% TODO - make a progr that data races immediately after thr create, no mutex es or whatever.
% TODO - check if it rly happens under pure happens before? or if this is only a concern under limited HB.
...
{\tt check\_withins()} implements the whitelist/blacklist behaviour for the former two of those commands
...
the reordered {\tt tmp2 <- x} is not directly part of the memory conflict which marked it,
but it must be executed first to reach the conflicting {\tt x <- tmp2}.
...
However, as the legend spoils, this subset of the state space still contains redundant interleavings,
arising from the fact that one pair of those 4 events is two reads, and hence not actually in conflict.
Visual inspection shows that $\bigstar$5, while locally justified in trying to reorder {\tt tmp1 <- x} before {\tt x <- tmp2},
effectively serves % oops
only to reorder it with {\tt tmp2 <- x}
relative to the first symmetric subtree.
Landslide identifies this equivalence by checking,
%in this case during the iteration of DPOR after interleaving K,
in this case after K,
that if the first event to be reordered (here, {\tt tmp <- x})
has already been tested in an equivalent reordering around any number of preceding events (here, {\tt tmp2 <- x} and {\tt tmp2++}),
then the newly marked subtree is safe to prune.
The next section describes the implementation of this check in general.

\subsubsection{Sleep set reduction}
\label{sec:landslide-sleepsets}

% TODO: condense this by removing the 'new' example and just continuing with the previous section's
...
For example, consider the execution sequence
in Figure~\ref{fig:sleepsets}(a).
After executing this interleaving, DPOR will detect the conflict between $t_a$ and $t_c$,
and tag $I_{a2}$ (i.e., $T_2$ to run before $t_a$), causing us to next execute
Figure~\ref{fig:sleepsets}(b).
However, DPOR does not ``remember'' why it wanted to reorder $T_2$ with $T_1$
(i.e., whether $t_b$ or $t_c$ was the important one to run before $t_a$),
so from this branch,
it next tags $I_{c'1}$, causing
Figure~\ref{fig:sleepsets}(c).
to be tested next, which is regrettably equivalent to the original interleaving
($t_a$ and $t_b$ having been reordered all told, which are independent).
To recover our forgotten original purpose,
which was already fulfilled by testing the second interleaving,
DPOR can check just before tagging a new interleaving (here, $I_{c'1}$),
among all preceding transitions equivalent to the conflicting one (here, $t_{b'}$ equivalent to $t_{a'}$),
for an already-explored interleaving beginning with the target thread.
If such exists,
%then because of the depth-first search ordering,
the new interleaving is guaranteed to be equivalent to one already checked,
and can be skipped.
...
\begin{figure}[h]
	...
	\\
	\\
	(a) Original interleaving.
	&
	(b) Goal interleaving.
	&
	(c) Redundant interleaving.
	\end{tabular}
	\end{center}
	\caption{Motivating example for the sleep sets optimization.
	For clarity, threads are labeled here with numbers ($T_n$) and the transitions thereby with letters ($t_i$).}
	\label{fig:sleepsets}
\end{figure}
...
% TODO: make sure it's clear that the quicksand chapter expts were run before this optmz was added,
% and that the HTM expts fro msigbovik paper were re-run for its chapater herein
...
% TODO: once dpor example tree is made
% switch this to that
As a concrete example, consider the state space of Figure~\ref{fig:tree}(b) (untouched by DPOR),
and suppose each transition to the next preemption point takes 1 second to execute.
While the first branch executes in 5 seconds,
the second branch, sharing the first transition as a common prefix, takes 4 seconds,
and the one after that only 2;
the state space being ultimately completed in 33 seconds.
Even with perfect hindsight,
na\"ively multiplying the total interleavings (10)
by the total execution time per branch (5) would double-count common prefixes
and grossly overestimate (50) the total runtime.
...
\begin{algorithm}[h]
	\For{$n \in [0..\mathsf{max}_P(U)]$}{
		\ForEach{$S \in U, P(S) \le n$}{
			Execute($S$)
		}
	}
