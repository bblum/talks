...
\begin{table}[h]
	\begin{center}
		...
		\begin{tabular}{cc||r|r||r|r||r|r}
			...
			& 3,1 & \cpu{2359.85}	& \ints{59850}	& \cpu{}	& \ints{}	& \cpu{}	& \ints{} \\
			\hline
			{\tt fig63}
			& 2,1 & \cpu{6.66}	& \ints{18}	& \cpu{6.98}	& \ints{6}	& \cpu{3.10}	& \ints{1} \\
			...
			& 2,2 & \ETAdag{133y 2d}	& \ETAdag{51484570} 	\\ % added 10hr but dosent matter
			...
			\hline
			{\tt map\_basicer}
			& 2,1 & \cpu{36.17}& \ints{426}	\\ % TODO make sure this gets updated if you update the test
	...
	\caption{Transactional tests verified (or not) by Landslide.
		...
		``Retry aborts only'' always tests every abort path and does not distinguish among failure reasons,
		as the baseline;
