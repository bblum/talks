...
still state their theorems and proofs in prose \cite{bpor,optimal-dpor,tsopso,satcheck,mcr,quicksand,cpp-model-checking},
...
\begin{table*}[t]
	...
	\begin{tabular}{cc||r|r|r||r|r|r|r}
		...
		\hline
		{\tt swapbug}
			& 2,1	&\cpu{104.37}&\wtm{17.00}&\ints{*6}& \cpu{*43.76}& \wtm{*12.51}& \ints{33	}& \ETA{73} \\
		(deadlock)
			& 2,2	&\cpu{ 66.47}&\wtm{*12.42}&\ints{*38}& \cpu{*45.22}& \wtm{15.30}& \ints{85	}& \ETA{860} \\
			& 2,3	&\cpu{ 99.41}&\wtm{*15.92}&\ints{*48}& \cpu{*53.57}& \wtm{21.34}& \ints{217	}& \ETA{9120} \\
			& 2,4	&\cpu{125.23}&\wtm{*19.46}&\ints{*68}& \cpu{*71.01}& \wtm{38.86}& \ints{537	}& \ETA{91239} \\
		\cline{2-9}
			& 3,1	&\cpu{313.36}&\wtm{*44.50}&\ints{*34}&\cpu{*78.23}& \wtm{60.49}& \ints{1016	}& \ETA{3543} \\
			& 3,2	&\cpu{*6500.36}&\wtm{*816.27}&\ints{*262}&\ETA{--}&\ETA{>1h}&\ETA{--}& \ETA{2161715} \\
		\hline
		{\tt avl\_insert}
			& 2,2	&\cpu{1355.05}&\wtm{174.09}&\ints{*29}	&\cpu{*93.28}&\wtm{*30.37}&\ints{79}&\ETA{157286} \\
		(segfault+)
			& 2,3	&\cpu{402.91}&\wtm{*54.60}&\ints{*33}&\cpu{*228.16}&\wtm{125.89}&\ints{835}&\ETA{13664151} \\
			% TODO: these 2,3 ones alone are new + ok
			% 33 interleavings; 1 preemptions; job time 12s; pldi time 179368977; new-fixed pldi cputime 1403360474
			% 835 interleavings; 1 preemptions; job time 1m 42s; pldi time 135292827; new-fixed pldi cputime 235664920
			& 2,4	&\ETA{--}&\ETA{>1h}&\ETA{--}&\ETA{--}&\ETA{>1h}&\ETA{--}& \ETA{79020639} \\
			% this quicksand jerk required ulimit -n higher than 1024
			%
			% the jerk didn't even find the bug anymore, after dpor-preferred tid
			% heres the stats from how it did before
			% & 2,4	&\cpu{*7598.19}&\wtm{*970.67}&\ints{*2633}&\ETA{--}&\ETA{>1h}&\ETA{--}& \ETA{79020639} \\
			% anyway this was the maximal
			%
		\cline{2-9}
			& 3,1	&\cpu{*86.43}&\wtm{*14.10}&\ints{*23}	&\cpu{91.97}&\wtm{24.80}&\ints{53}&\ETA{343250} \\
			& 3,2	&\cpu{*2177.86}&\wtm{*287.66}&\ints{*169}&\ETA{--}&\ETA{>1h}&\ETA{--}& \ETA{425939133} \\
			% TODO
			%\multicolumn{9}{c}{everything above this line needs rerun for dpor-preferred tid + thr_bottom1} \\
...
\begin{table}[p]
	\begin{center}
		...
		\begin{tabular}{cc||r|r||r|r||r|r}
			...
			\hline
			% TODO: everything above this line needs rerun
			% TODO: everything below is fine up to dpor-preferred & thr_bottom1 updates
			{\tt fig63}
			...
			\hline
			{\tt map\_basicer}
			& 2,1 & \cpu{28.40} 	& \ints{150} 	& \cpu{94}	& \ints{26.55}	& \cpu{14.88}	& \ints{9} \\
			& 2,2 & \ETAdag{qqq}	& \ETAdag{qqq}	& \ETAdag{qqq}	& \ETAdag{qqq}	& \cpu{1285.06}	& \ints{21684} \\
			& 3,1 & \ETAdag{qqq}	& \ETAdag{qqq}	& \ETAdag{qqq}	& \ETAdag{qqq}	& \ETAdag{qqq}	& \ETAdag{qqq} \\
	...
	\caption{Transactional tests verified (or not) by Landslide.
		...
		{\em Thesis draft note: I need to rerun these tests to ensure consistent cpu times with the next table,
		but I expect the state space sizes not to change substantially.}
		}
	\label{tab:tm-verifs}
\end{table}
...
\begin{table}[t]
	\begin{center}
		...
		\begin{tabular}{cc||r|r}
			...
			& 3,2 & \ETAdag{qqq} & \ETAdag{qqq} \\
			& 4,1 & \cpu{qqq} & \ints {qqq} \\ % was: 30408, 282084 pre mkrun
			\hline
			{\tt lock}
			& 2,1 & \cpu{3.44} & \ints{4}		\\
			({\tt mutex})
			...
			& 2,4 & \cpu{qqq} & \ints{qqq}	\\ % was: 15266 sec 384512 pre mkrun
			...
			& 2,2 & \cpu{qqq} & \ints{qqq} \\ % 30k and rising, looks like mb 5 hr? or 10+hr. 50/50 odds
			& 2,3 & \ETAdag{qqq} & \ETAdag{qqq} \\
			& 3,1 & \cpu{qqq} & \ints{qqq} \\ % 200k and rising, same tbh
			& 3,2 & \ETAdag{qqq} & \ETAdag{qqq} \\
			& 4,1 & \ETAdag{qqq} & \ETAdag{qqq} \\
			\\
			\\
			\\
			\\
...
than {\tt mutex}
% TODO recalc
(roughly 9 interleavings per second for the former, 14 for the latter),
