...
\begin{table}[p]
	\begin{center}
		...
		\begin{tabular}{cc||r|r||r|r||r|r}
			...
			& 2,3 &\cpu{qqq}	& \ints{qqq}	& \cpu{qqq}	& \ints{qqq}	& \cpu{910.80}	& \ints{22409} \\
			...
			& 4,2 & \cpu{qqq} 	& \ints{qqq} 	& \cpu{qqq}	& \ints{todo atm}	& \cpu{3.28}	& \ints{1} \\
			% measured with !CONSIDER_ONLY_MOST_RECENT_DPOR_PREFERRED_TID
...
(i.e., $({\sf txn}+{\sf sync}+{\sf race})*K*N$ for the highest $(K,N)$ completed.
...
with no use of unbounded-capacity data structures or 
...
(none of the bugs in \sect{\ref{sec:tm-eval-bugs}} required any higher parameters than $(2,2)$ to expose).
...
% TODO
% discuss the diff reducion approaches

% Please note that retry sets are a theoretically-founded reduction that should apply to all programs,
% while distinguishing abort codes to suppress retries is potentially unsound,
% depending on the user's ability to manually verify the presence (and correctness) of retry loops in the code.

% TODO

% discussion items

% note that even programs which have fully conflicting txn/abort paths, under SSS-MC abort sets have nothing to reduce,
% but once you split those abort paths apart by dr pps, abort sets become useful again
...
\begin{table}[h]
	\begin{center}
		...
		\begin{tabular}{cc||r|r}
			...
			({\tt spinlock})
			& 2,2 & \cpu{174.26} & \ints{1702} \\ % certified causality ok
			% if lock(spin_fixed) will time out on 2,3 and 3,2, this is guaranteed to aswell
			% & 2,3 & \\ % \cpu{0} & \ints{0} \\
			& 3,1 & \cpu{26.97} & \ints{246} \\ % certified causality ok
			% & 3,2 & \\ % \cpu{0} & \ints{0} \\
			...
			\hline
			{\tt lock}
			& 2,1 & \cpu{2.47} & \ints{4} \\ % certified causality ok
			({\tt spin\_fixed})
			& 2,2 & \cpu{94.64} & \ints{998} \\ % certified causality ok
			& 2,3 & \ETAdag{28h 6m} & \ETAdag{486966} \\ % certified causality ok, add 10h ok
			% [JOB 4] progress: 234307/486966 brs (48.115630%), ETA 18h 5m 37s (elapsed 9h 59m 50s)
			& 3,1 & \cpu{21.68} & \ints{186} \\ % certified causality ok
			...
			& 4,1 & \cpu{493.38} & \ints{15064}	\\ % certified causality ok
			%%%%%%%%%%% htm_mutex
