...
we did not actually implement an HTM-style speculative-execution-and-rollback simulation mode.
% so have no basis to measure how much it saves, or smth like that
On this count, at least,
we hope the reader finds it self-evident that
our equivalence proofs provide exponential state space reduction
...
\begin{enumerate}
	...
	\item How does Iterative Deepening's performance compare to Maximal State Space mode?%
...
\begin{itemize}
	...
	\item Lock abstraction tests.
		...
		{\tt spinlock\_fixed} (spoilers!!),
		and {\tt mutex} (replaces the spinlock with a Landslide-annotated mutex to reduce state space size).
	\begin{itemize}
...
\begin{table*}[t]
	...
	\caption{Landslide's bug-finding performance on various test configurations.
		Iterative Deepening, optimized for fast bug-finding,
		is compared against Maximal State Space mode,
		...
		* marks the winning measurements between each series.
		Lastly, state space estimation, % \sect{\ref{sec....}}
...
% TODO: better sec'n reference?
I configured Landslide to run Quicksand's Iterative Deepening algorithm (\sect{\ref{chap:quicksand}}),
shown left,
...
\begin{figure}[t]
	\begin{center}
		...
		\texttt{~~~~return (\_\_sync\_bool\_compare\_and\_swap(\&lock->v, 0, 0) ? true : false);} \\
		\texttt{\}} \\
		...
		\texttt{~~~~~~~~\_xabort(0xff);} \\
		\texttt{~~~~\} else \{} \\
		\texttt{~~~~~~~~// ... retrying, etc. abbreviated for brevity ...} \\
...
% (2,2) with isfree fix
%%%% total time elapsed: 25m 1s
%%%% [JOB 4] COMPLETE (15076 interleavings tested; 24m 41s elapsed)
%%%% total CPU time consumed: 25m 4s (1504809436 usecs) (core saturation: 25%)
% (nb. it's not known whether this was run from a "hot" start (no recompiling) -- but probably wasnt!
...
%%%%%%%%%%% htm_mutex
%%%% -- nb. these numbers were obtained with a oops-exploded state space from a misplaced assert on hm->held
% % 2,1
...
% such as in a program which requires a transaction abort rate below a certain threshhold
% owing to the nature of its workload,
% because all interleavings are equal in Landslide's eyes, and probability is no object.
% However, the lock_noconflict test illustrates that model checking can still test for certain performance properties
