...
entirely discarded with an error code returned to allow the programmer to \revisionminor{invoke} a synchronized backup path.
%
...
\begin{figure}[h]
	\begin{center}
		\begin{tabular}{ll}
			...
			%2 & \texttt{~~~~x++;} \\
			$2a$ & \texttt{~~~~temp <- x;} \\
			$2b$ & \texttt{~~~~temp <- temp + \const{1};} \\
			$2c$ & \texttt{~~~~x <- temp;} \\
			3 & \texttt{~~~~\call{\_xend}();} \\
			4 & \texttt{\} \flow{else} \{} \\
			5 & \texttt{~~~~\call{mutex\_lock}(\&m);} \\
			$6a$ & \texttt{~~~~temp <- x;} \\
			$6b$ & \texttt{~~~~temp <- temp + \const{1};} \\
			$6c$ & \texttt{~~~~x <- temp;} \\
			7 & \texttt{~~~~\call{mutex\_unlock}(\&m);} \\
			8 & \texttt{\}} \\
		\end{tabular}
...
\begin{figure}[h]
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption[More correct version of the program in \Cref{fig:htm-example-reproduced}.]
		{More correct version of the program in \Cref{fig:htm-example-reproduced},
...
\Cref{thm:atom} must be weakened correspondingly, yielding \Cref{thm:atom-weak}.
\vspace{1em}
...
\qrevision{This proof assumes that all operations $\tiiat{\beta}$
%within $\tii$'s
are in fact executed transactionally.
In STM systems, the user must annotate all shared memory accesses she wishes to be protected,
so an incorrectly-annotated program could mix transactional and non-transactional operations
in $\tii$.
To emulate the interactions thereof with other threads would require fully simulating failure rollback after all,
which I must leave to future work.
I believe this assumption to be justified for the sake of any future weakly-atomic HTM systems,
in which the user would not have the mixed blessing of such fine-grained annotations.}
...
As one might expect, it inflates state spaces considerably,
and moreover,
...
\begin{figure}[t]
	\begin{center}
		\begin{tabular}{l}
		\texttt{\flow{while} (\_retry) \flow{continue};} \\ % in the actual code it's "while (retry);"; i'm adding continue for clarity
...
\begin{figure}[h]
	...
	\begin{tabular}{p{0.47\textwidth}p{0.5\textwidth}}
		\begin{tabular}{l}
			\texttt{\ctype{static int} num\_in\_section = \const{0};} \\
...
\begin{table}[hb!] % ugh... this placement directive... (be on same page as above figure)
	\begin{center}
		\begin{tabular}{cc}
		\footnotesize
		...
		\footnotesize
			...
			% [JOB 0] progress: 764124/23136172 brs (3.302724%), ETA 11d 11h 45m 28s (elapsed 9h 59m 59s)
			& 4,1 & \cpu{199.96} & \ints{4488} \\
			& 4,2 & \ETAdag{41d 0h} & \ETAdag{66520074} \\
			% [JOB 0] progress: 702872/66520074 brs (1.056631%), ETA 40d 13h 44m 41s (elapsed 9h 59m 59s)
			\cline{1-4}
			{\tt map\_mutex}
	...
	\caption[Continuation of \Cref{tab:tm-verifs}, demonstrating abstraction reduction.]
		...
		\\
		% this above extra vspace necessary so that this table and fig6.9 get a page all to themselves,
		% without latex trying to sneak two lines of body prose in at the bottom or between them
		}
...
This section will review %some of
...
\begin{itemize}
	\item An approach for model checkers to emulate strongly-atomic HTM
		by treating transactions as globally-uninterruptible atomic sections
		...
		plus an extension to also emulate weakly-atomic STM systems
		by relaxing the above atomicity to apply to other transactions only
		(\cref{sec:tm-weak-atomicity}),
		each section with a corresponding soundness proof of equivalence to existing systems' semantics.
