...
\newcommand\weakhilight[1]{{\ul{#1}}\xspace}
...
However, under weakly-atomic STM systems,
transactions do not abort when
a non-transactional conflicting access interleaves within them,
and condition \#1 of the execution semantics from \cref{sec:tm-design-failure} does not hold.
To model weak atomicity, we must weaken \Cref{lem:equiv} as follows (updated parts underlined for emphasis).
\vspace{1em}

\begin{lemma}[Equivalence of Aborts \weakhilight{under Weak Atomicity}]
	...
	with $b<n$, $j \ne i$, $k \ne i$, et cetera, either:
	\begin{enumerate}
		\item \weakhilight{All events in
			$\tjjat{\gamma_1}\dots\tjjat{\gamma_j},
			 \tkkat{\kappa_1}\dots\tkkat{\kappa_k}$
			 % ehh, bad parallel structure with below regarding the verb 'exists'
			are non-transactional, and the original interleaving itself exists, or}
		\item $\tiiat{\alpha},\tjjat{\gamma_1}\dots\tjjat{\gamma_j},\tkkat{\kappa_1}\dots\tkkat{\kappa_k},\tiiat{\phi_1}\dots$
			(conflicting case), or
		\item $\tiiat{\alpha},\tiiat{\beta_1}\dots\tiiat{\beta_b}\dots\tiiat{\beta_n},\tjjat{\gamma_1}\dots\tjjat{\gamma_j},\tkkat{\kappa_1}\dots\tkkat{\kappa_k}$
			(independent case)
...
\begin{proof}
	As before, \weakhilight{except first case on
	whether any intervening operations by $\tjj$/$\tkk$ are transactional.%
	\footnote{If $\tjj$/$\tkk$ already have a transaction active,
	...
	If not, $\tii$ may ultimately proceed with $\tiiat{\beta_{b+1}}$ without aborting,
	and the new case \#1 applies.}
\end{proof}

\Cref{thm:atom} must be weakened correspondingly.
\vspace{1em}

\begin{theorem}[\weakhilight{Weak} Atomicity of Transactions]
	\label{thm:atom-weak}
	For any state space $S$ of a transactionally-concurrent program,
	an equivalent state space exists in which all transactions are either executed atomically
	\weakhilight{with respect to other transactions only},
...
Conveniently, in \cref{sec:tm-design-locks}'s formulation of
...
\begin{itemize}
	...
	\item {\tt \_XABORT\_CAPACITY}:
		...
		\footnote{As far as I know, the reason for syscall aborts is both hardware- and kernel-dependent,
...
because I know of no existing HTM system which implements it.
...
% TODO: is this true? at all? htm2 is bigger in STM only because it's got a separate expliit abort
