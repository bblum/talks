...
Use of {\em italics} within definitions will indicate terms also defined in this list.

\begin{enumerate}
	\item {\bf API.}
		Short for {\em application programming interface} (which nobody ever says).
		% TODO
	\item {\bf Atomic.}
	...
	\item {\bf False positive.}
		...
		are both heuristic (\sect{\ref{sec:landslide-blocking}}),
	...
	\item {\bf Iterative Deepening.}
		The heuristic search ordering strategy used by {\em Quicksand}.
	...
	\item {\bf Lock.}
		% TODO
	\item {\bf Memory conflict.} See {\em conflict}.
	\item {\bf mutex.}
		Short for {\em mutual exclusion lock}. The name {\em P2} and {\em pthreads} use for their {\em lock API}.
	...
	\item {\bf Race condition.}
		A confusing term which, while CMU 15-410 teaches it to mean {\em concurrency bug},
		in research circles is generally taken to mean {\em data race}.
