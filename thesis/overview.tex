...
\begin{itemize}
	...
	\item Condition variables, with the interface {\tt cond\_wait(cvp, mp)}, {\tt cond\_signal(cvp)}, and {\tt cond\_broadcast(cvp)}. {\tt cond\_wait()} blocks the invoking thread, ``simultaneously'' releasing a mutex which protects some associated state (atomically, with respect to other calls under the same mutex).
...
\begin{enumerate}
	...
	\item \revision{{\bf Relaxed memory.}
		...
		% TODO: check that quicksand discussion shouldn't perhaps get a more subsection'ed sect reference
