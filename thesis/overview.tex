...
\revisionminor{This chapter introduces the necessary background material on concurrency
(\cref{sec:overview-concurrency}),
stateless model checking,
(\cref{sec:overview-stateless}),
data race analysis
(\cref{sec:background-datarace}),
and the relevant undergraduate operating systems classes
(\cref{sec:overview-edu}).
It concludes with a glossary of terminology
(\cref{sec:glossary})
that I hope the reader may find convenient as a quick reference
while reading later chapters as well.}
...
\revision{This section presents an elementary introduction to
%the fundamentals of
concurrent programming.
I have taken care to make it accessible to readers with even only basic programming experience,
familiar with flow control, functions, memory, and objects at most,
although I cannot avoid the inherent difficulty of multiple layers of abstraction building upon one another.
For an explanation to be ``elementary''
means that it requires very little be known ahead of time to approach it,
not necessarily that it be easy
\cite{lost-lecture}.
%\footnote{``Elementary'' does not mean easy to understand.
%``Elementary'' means that very little is required to know ahead of time in order to understand it,
...
Consider the program in \Cref{fig:concurrency-bug}: Any output between 2 and 2000 is possible\footnote{
...
\begin{figure}[t]
	\begin{tabular}{cc}
		\begin{tabular}{p{0.44\textwidth}p{0.495\textwidth}}
			{
			\begin{tabular}{l}
		...
		\end{tabular}
	\end{tabular}
	\caption[Example concurrent program.]{Example concurrent program in which simultaneous accesses to shared state may interleave to produce unexpected results.}
...
To prevent unexpected interleavings such as the example in \Cref{fig:concurrency-bug}(b),
...
\begin{figure}[t]
	\begin{tabular}{p{0.45\textwidth}p{0.5\textwidth}}
		...
		&
		(b) The {\tt count} function from \Cref{fig:concurrency-bug}, adjusted to use a mutex to ensure each increment of {\tt x} is uninterruptible.
	\end{tabular}
	\caption[Using a locking primitive to protect accesses to shared state.]
		{Using a locking primitive to protect accesses to shared state.}
	\label{fig:mutex}
\end{figure}
...
\revisionminor{see \cref{sec:landslide-config-landslide}})
at the cost of increasing the implementation and theoretical complexity of the analysis
\revisionminor{(\cref{sec:tm-abstraction})}.
...
\Cref{fig:mutex}(a) shows how a yielding mutex (not the best implementation, but the simplest) may be implemented using {\tt xchg},
and (b) shows how a mutex may be used to fix the example from \Cref{fig:concurrency-bug}.
...
The example program in \Cref{fig:htm-example} demonstrates how these primitives can be used to synchronize a simple shared access without locking overhead in the common case\footnote{
	The solution presented here is actually incomplete; stay tuned until \Cref{chap:tm} for the surprising twist!
},
...
\begin{figure}[t]
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption[The example {\tt count} routine from \Cref{fig:mutex}, rewritten to use HTM.]
		{The example {\tt count} routine from \Cref{fig:mutex}, rewritten to use HTM.
...
\Cref{chap:tm} will explore the consequences of this difference further.
%
...
To visualize this,
using a single iteration of the {\tt x++;} loop from \Cref{fig:concurrency-bug} as an example,
with {\tt x++;} expanded into its three corresponding pseudo-assembly instructions,
\Cref{fig:tree}(a) shows all possible execution interleavings
...
\begin{figure}[p]
	...
	\end{tabular}
	\caption[Visualization of interleaving state space for the program in \Cref{fig:concurrency-bug}.]
		{Visualization of interleaving state space for the program in \Cref{fig:concurrency-bug}.
...
\Cref{fig:tree}(b) shows a visualization of the corresponding tree from our example program,
...
\revisionminor{which this thesis will address in \Cref{chap:quicksand}}.
...
\revisionminor{To define it abstractly, let}
{\em independent transitions} denote a pair of executions of two threads, each from one preemption point to the next,
...
\revisionminor{For a friendlier, more concrete example, consider \Cref{fig:dpor}, which}
...
\revisionminor{Any events executed}
before the thread 1/thread 2 sequence,
any other runnable threads besides these two, %threads 1 or 2,
...
Iterative Deepening, presented in \Cref{chap:quicksand}, is another such search heuristic.
...
\end{enumerate}
%In \Cref{fig:example}, lines 3 and 5 each race with 2 and 6, and line 6 races with 8.
In \Cref{fig:hb-example}, the pairs of lines marked with comments (A1 and A2, B1 and B2) race.
...
A data race analysis may be either {\em static} (inspecting source code) \cite{racerx}
...
as shown in the example program in \Cref{fig:hb-example}(a).
...
However, consider the similar program in \Cref{fig:hb-example}(b),
...
Causally-Precedes will identify the potential race in \Cref{fig:hb-example}(a), as the two critical sections do not conflict,
...
\Cref{chap:quicksand} includes a comparison of the two approaches for the purpose of finding new preemption points for model checking.
...
Both P2 and P3 are built using the {\em Pebbles} system call specification, outlined in \Cref{tab:syscalls}.
...
\begin{itemize}
	...
	\item Semaphores, with the interface {\tt sem\_wait(sp)} and {\tt sem\_\allowbreak{}signal(sp)}
		(sometimes also called \revisionminor{$P$ and $V$ in other literature,
		from Dutch,
	...
	\item Reader-writer locks (also called R/W-locks or rwlocks), with the interface {\tt rwlock\_\allowbreak{}lock(rwp, mode)} and {\tt rwlock\_unlock(rwp)}. {\tt mode} may be either {\tt RWLOCK\_READ} or {\tt RWLOCK\_\allowbreak{}WRITE}.
...
\subsubsection{P3}
In P3, students implement a kernel which provides the same system calls shown in \Cref{tab:syscalls}, previously provided by the reference kernel.
...
% haha-never: Talk about the hurdle (both for p2 and p3).
...
The Pintos kernel architecture \cite{pintos} is used at several universities, including the University of Chicago, Berkeley, and Stanford.
...
\begin{itemize}
	\item {\em Threads}: Students must implement an ``alarm clock'' (analogous to Pebbles's {\tt sleep} system call),
		...
		% haha-never: Later. Talk about how concurrency testing can only test certain parts of this crap.
...
\begin{enumerate}
	...
	\item {\bf Execution tree.}
		...
		See \cref{sec:overview-statespace}; \Cref{fig:tree}.
	...
	\item {\bf Landslide.}
		...
		Documented in \Cref{chap:landslide}.
	...
	\item \revision{{\bf misbehave.}
		...
		is kept secret to preserve the intended challenge of debugging (see \cref{sec:410-secrecy}).}
	\item {\bf mutex.}
	...
	\item \revision{{\bf Partial store order.} See {\em relaxed memory.}}
	\item \revision{{\bf Pebbles.}
		The educational kernel architecture used in 15-410 at CMU. See \cref{sec:pebbles}.}
	\item \revision{{\bf Pintos.}
		The educational kernel architecture used in \uchos at the University of Chicago,
		as well as several other universities. See \cref{sec:overview-pintos}.}
	\item {\bf pthreads.}
		POSIX thread library for Linux programs. %provided by POSIX.
		\revision{The modern pthreads model traces its lineage back to
		DEC's Systems Research Center threads \cite{src-threads}
		and Mach's C threads \cite{mach-c-threads},
	...
	\item \revision{{\bf PSO.} {\em Partial store order}. See {\em relaxed memory}.}
	...
	\item {\bf Race condition.}
		...
		\footnote{In the author's personal experience, many readers of their M.S. thesis \cite{landslide}
	...
	\item \revision{{\bf Relaxed memory.}
		Also known as {\em weak memory}.
		...
		For further discussion see \cref{sec:quicksand-soundness}, \cref{sec:tm-warpzone-relaxed}, and \cref{sec:related-other-verif}.}
	\item {\bf Schedule.}
	...
	\item \revision{{\bf Total store order.} See {\em relaxed memory.}}
	\item \revision{{\bf TSO.} {\em Total store order}. See {\em relaxed memory.}}
	\item {\bf Userspace.}
	...
	\item \revision{{\bf Weak memory.}
		See {\em relaxed memory.}}
\end{enumerate}
